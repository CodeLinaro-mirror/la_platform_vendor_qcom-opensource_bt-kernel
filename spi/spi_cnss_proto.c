...
void spi_cnss_kfree(struct spi_cnss_priv *spi_drv, void **ptr)
{
    ...
    if (*ptr) {
        ...
        *ptr = NULL;
        ...
        SPI_CNSS_ERR(spi_drv, "%s: Ptr is null. Nothing to free \n", __func__);
    }
    mutex_unlock(&spi_drv->mem_lock);
}
...
//#else
		...
		if (!nop_cmd_buf) {
			SPI_CNSS_ERR(spi_drv, "%s: Mem allocation failed\n",__func__);
			return -ENOMEM;
		}
...
//#ifndef MEM_ALLOCATOR
	if (nop_cmd_buf) {
		spi_cnss_kfree(spi_drv, (void **)&nop_cmd_buf);
	}
//#endif
	return ret;
}
...
//	struct spi_transfer *xfer =  spi_cnss_kzalloc(spi_drv, (sizeof(struct spi_transfer)*3));
	...
	if (ret < 0) {
		SPI_CNSS_ERR(spi_drv, "%s: failed prepare write notifier",__func__);
...
//#ifndef MEM_ALLOCATOR
		if (!spi_drv->mem_mngr.tx_payload)
			spi_cnss_kfree(spi_drv, (void **)&tx_buf);
//#endif
		return ret;
	}
	return 0;
...
//#ifndef MEM_ALLOCATOR
	...
	!spi_drv->mem_mngr.clen_notifier_two) {
		spi_cnss_kfree(spi_drv, (void **)&txbuf);
		spi_cnss_kfree(spi_drv, (void **)&txbuf1);
	}
//#endif
	return ret;
}
...
//#ifndef MEM_ALLOCATOR
	if (!spi_drv->mem_mngr.len_tx_buf || !spi_drv->mem_mngr.len_rx_buf) {
		spi_cnss_kfree(spi_drv, (void **)&clen_tx_buf);
		spi_cnss_kfree(spi_drv, (void **)&clen_rx_buf);
	}
//#endif
	return ret;
}
...
int spi_cnss_wakeup_client(struct spi_cnss_priv *spi_drv, int retry)
{
	...
	if (spi_drv->client_state != AWAKE) {
		ret = spi_cnss_register_xfer(spi_drv, SPI_SLAVE_SANITY_REG, SPI_REGISTER_READ);
		if (ret < 0) {
			SPI_CNSS_ERR(spi_drv, "%s: Controller is in bad state or not powered on: %d\n",__func__, ret);
			spi_drv->client_state = ASLEEP;
			ret = -1;
		}
	}
...
//#else
			...
			if (!tx_buf) {
				SPI_CNSS_ERR(spi_drv, "%s: failed to alloc rx buf\n",__func__);
				spi_cnss_kfree(spi_drv, (void **)&rx_buf);
				return -ENOMEM;
			}
		}
...
//#endif
		...
		SPI_CNSS_ERR(spi_drv, "%s: No active client, bailing\n",__func__);
		spi_drv->read_pending = false;
	}
...
static void spi_cnss_read_msg(struct kthread_work *work)
{
	...
	if (spi_drv->client.CBUF_LEN > 0) {
		...
		SPI_CNSS_ERR(spi_drv, "%s: CBUF LEN is cleared, ignore read request\n",__func__);
	}
...
static void spi_cnss_free_allocated_memory(struct spi_cnss_priv *spi_drv)
{
	...
	spi_cnss_kfree(spi_drv, (void **)&spi_drv->mem_mngr.sleep_cmd_buf);
...
static int spi_cnss_allocate_memory(struct spi_cnss_priv *spi_drv)
{
	...
	if (!spi_drv->mem_mngr.notifier_one) {
		SPI_CNSS_ERR(spi_drv, "%s:notifier_one failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.notifier_two) {
		SPI_CNSS_ERR(spi_drv, "%s:notifier_two failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.nop_cmd_buf) {;
		SPI_CNSS_ERR(spi_drv, "%s:nop_cmd_buf failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.sleep_cmd_buf) {
		SPI_CNSS_ERR(spi_drv, "%s:sleep_cmd_buf failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.rx_cmd_buf) {
		SPI_CNSS_ERR(spi_drv, "%s:rx_cmd_buf failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.len_tx_buf) {
		SPI_CNSS_ERR(spi_drv, "%s:len_tx_buf failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.len_rx_buf) {
		SPI_CNSS_ERR(spi_drv, "%s:len_rx_buf failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.clen_notifier_one) {
		SPI_CNSS_ERR(spi_drv, "%s:clen_notifier_one failed\n",__func__);
		ret = -ENOMEM;
		goto err;
	...
	if (!spi_drv->mem_mngr.clen_notifier_two) {
		SPI_CNSS_ERR(spi_drv, "%s:clen_notifer_two failed\n",__func__);
		ret = -ENOMEM;
		goto err;
...
static int spi_cnss_read_context_info(struct spi_cnss_priv *spi_drv, bool is_irq)
{
	...
	if (is_irq) {
		...
		if ((ret >= 0) && spi_drv->state_transition &&
			...
			ret = spi_cnss_read_len(spi_drv);
			if (ret < 0) {
				SPI_CNSS_ERR(spi_drv, "%s: read len failed\n",__func__);
				return ret;
			}
...
//#else
			...
			if (!txbuf) {
				SPI_CNSS_ERR(spi_drv, "%s:tx memory alloc failed for context info\n",__func__);
				return -ENOMEM;
			}

			rxbuf = spi_cnss_kzalloc(spi_drv, CONTEXT_INFO_READ_SIZE);
			if (!rxbuf) {
				SPI_CNSS_ERR(spi_drv, "%s:rx memory alloc failed for context info\n",__func__);
				return -ENOMEM;
			}
		}
//#endif
...
#endif
		if (ret < 0) {
			SPI_CNSS_ERR(spi_drv, "%s: context info read failed\n",__func__);
			spi_cnss_kfree(spi_drv, (void **)&txbuf);
										...
										struct spi_cnss_user *usr, size_t len)
{
	...
	if (timeout <= 0) {
		SPI_CNSS_ERR(spi_drv, "%s: err timeout for sync_wait\n",__func__);
		return -ETIMEDOUT;
	}
...
//#else
		tx_buf = spi_cnss_kzalloc(spi_drv, REG_TX_SIZE);
		if (!tx_buf) {
			ret = -ENOMEM;
			SPI_CNSS_ERR(spi_drv, "%s: Mem Alloc Failed:\n",__func__);
			return ret;
		}
		rx_buf = spi_cnss_kzalloc(spi_drv, REG_RX_SIZE);
...
#endif
	if (ret < 0) {
		SPI_CNSS_ERR(spi_drv, "%s: reg rd/wr failed = %d\n",__func__, ret);
	} else {
...
static int spi_cnss_controller_init(struct spi_cnss_priv *spi_drv)
{
	...
	if (spi_drv->client_state == AWAKE) {
		...
		if (ret < 0) {
			SPI_CNSS_ERR(spi_drv, "%s: SPI_SLAVE_DEVICE_ID_REG read failed: %d\n",__func__, ret);
			return ret;
		}
		SPI_CNSS_DBG(spi_drv,"%s: slave dev id read\n",__func__);
		ret = spi_cnss_register_xfer(spi_drv, SPI_SLAVE_STATUS_REG, SPI_REGISTER_READ);
		if (ret < 0) {
			SPI_CNSS_ERR(spi_drv, "%s: SPI_SLAVE_STATUS_REG read failed: %d\n",__func__, ret);
			return ret;
		}
		...
		if (ret < 0) {
			SPI_CNSS_ERR(spi_drv, "%s: spi_cnss_read_context_info, init failed: %d\n",__func__, ret);
			return ret;
		}
	}
	return ret;
...
static int spi_cnss_open(struct inode *inode, struct file *filp)
{
	...
	SPI_CNSS_DBG(spi_drv, "%s rc =%d PID =%d\n", __func__, rc, current->pid);
#ifdef MEM_ALLOCATOR
	...
	if (!spi_drv->client_irq_buf) {
		spi_drv->client_irq_buf = spi_cnss_kzalloc(spi_drv, IRQ_WRITE_SIZE);
		if (!spi_drv->client_irq_buf) {
			SPI_CNSS_ERR(spi_drv, "%s: client_irq_buf mem alloc failed\n", __func__);
			ret = -ENOMEM;
			goto end;
		}
	}
...
static ssize_t spi_cnss_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (atomic_read(&usr->rx_avail)) {
		...
		SPI_CNSS_ERR(spi_drv, "%s: No client packet avaiable, spurious read request",__func__);
		return -EINVAL;
	}
	return ret;
}
...
static int spi_cnss_resume(struct device *dev)
{
	...
	SPI_CNSS_DBG(spi_drv, "%s: calling runtime resume\n", __func__);
